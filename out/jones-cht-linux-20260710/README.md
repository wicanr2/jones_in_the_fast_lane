...
| 全文字對白／訊息 | 棋盤地點招牌 |
|---|---|
| ![對白](docs/images/m2-university-dialogue-cht.png) | ![棋盤](docs/images/m3-board-cht.png) |
| **動作按鈕** | **啟動版權畫面** |
| ![按鈕](docs/images/m3-buttons-cht.png) | ![版權](docs/images/m1-spike-copyright-cht.png) |

- **文字**：對白、選單、訊息、動態計數（如「第 N 週」）全繁體中文——**776 則**翻譯。
- **烘字美術**：棋盤 12+ 地點招牌（工廠、銀行、當舖、租屋、Z超商…）、14 個動作按鈕（完成/工作/購買/出售…）、目標畫面橫幅（設定你的目標）重繪成中文。
- **字型**：文泉驛正黑 15px（現代黑體，貼合都市生活模擬調性）。
...
```bash
bash tools/apply_patches.sh <scummvm-src>   # 套引擎繪字 patch(0001+0002+fontchinese)
